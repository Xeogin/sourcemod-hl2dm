...
#define PLUGIN_AUTHOR		"Spartan_C001, Dragokas, Gemini+Xeogin"
#define PLUGIN_DESCRIPTION	"Plays sounds based on events that happen in game."
#define PLUGIN_VERSION		"3.5.7"
#define PLUGIN_URL			"https://steamcommunity.com/id/spartan_c001/"

/*
	3.5.7 (31-Jan-2026)
	 - Forked by Xeogin
	 - Removed random sound pitch
	 - Fixed headshot detection for HL2DM
	 - Set default ConVar values to 'Enabled' for better out-of-the-box usability
	 - Updated translations and added missing language entries
	 - Added HL2-based sound set and dynamic translation logic

	3.5.6 (26-Feb-2019)
	 - Added random sound pitch.
	 - Added !quakeplay menu to test every sound.
	
	3.5.5 (15-Feb-2019)
	 - Blocked "humilation" (selfkill) sound for coop-mode for infected team, because too many spam.
	 - Fixed and appended some paths in sound sets config.
	
	3.5.1 (20-Jan-2019)
	 - Fork by Dragokas
	 - Added translation into Russian, added missed "enabled" phrase.
	 - Added randomly "mixed" sound set.
	 - Added "sm_quakesoundsv3_text" fix by Xeogin
*/
...
// Hitgroup Detection
new g_LastHitGroup[MAXPLAYERS + 1]
...
// Sound Files
new String:headshotSound	[MAX_NUM_SETS+1][MAX_NUM_KILLS][PLATFORM_MAX_PATH]
new String:grenadeSound		[MAX_NUM_SETS+1][PLATFORM_MAX_PATH]
new String:selfkillSound	[MAX_NUM_SETS+1][PLATFORM_MAX_PATH]
new String:roundplaySound	[MAX_NUM_SETS+1][PLATFORM_MAX_PATH]
new String:knifeSound		[MAX_NUM_SETS+1][PLATFORM_MAX_PATH]
new String:killSound		[MAX_NUM_SETS+1][MAX_NUM_KILLS][PLATFORM_MAX_PATH]
new String:firstbloodSound	[MAX_NUM_SETS+1][PLATFORM_MAX_PATH]
new String:teamkillSound	[MAX_NUM_SETS+1][PLATFORM_MAX_PATH]
new String:comboSound		[MAX_NUM_SETS+1][MAX_NUM_KILLS][PLATFORM_MAX_PATH]
new String:joinSound		[MAX_NUM_SETS+1][PLATFORM_MAX_PATH]

// Sound Configs
new headshotConfig		[MAX_NUM_SETS+1][MAX_NUM_KILLS]
new grenadeConfig		[MAX_NUM_SETS+1]
new selfkillConfig		[MAX_NUM_SETS+1]
new roundplayConfig		[MAX_NUM_SETS+1]
new knifeConfig			[MAX_NUM_SETS+1]
new killConfig			[MAX_NUM_SETS+1][MAX_NUM_KILLS]
new firstbloodConfig	[MAX_NUM_SETS+1]
new teamkillConfig		[MAX_NUM_SETS+1]
new comboConfig			[MAX_NUM_SETS+1][MAX_NUM_KILLS]
new joinConfig			[MAX_NUM_SETS+1]
...
new g_iSelectedSet[MAXPLAYERS+1];
...
public OnPluginStart()
{
	...
	CreateConVar("sm_quakesoundsv3_version",PLUGIN_VERSION,"Version of currently loaded Quake Sounds v3 plugin.",FCVAR_NOTIFY|FCVAR_DONTRECORD)
	...
	CreateConVar("sm_quakesoundsv3_sound","-1","Default sound set for new users, -1=Mixed, 0=Disabled, 1=Standard, 2=Female.",FCVAR_NONE)
	...
	RegConsoleCmd("sm_quakeplay",CMD_ShowQuakePlayMenu)
	HookGameEvents()
...
public LateLoadedInitialization()
{
	...
	for(new i = 1; i <= GetMaxHumanPlayers(); i++) 
	{
		if(IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			if (soundPreference[i] == -2) soundPreference[i] = numSets > 0 ? numSets : -1;
...
void MixSets()
{
	for (int i = 0; i <	numSets; i++) {
		for (int j = 0; j < MAX_NUM_KILLS; j++) {
			strcopy(headshotSound[numSets][j], PLATFORM_MAX_PATH, headshotSound[GetRandomInt(0, numSets - 1)][j]);
			strcopy(killSound[numSets][j], PLATFORM_MAX_PATH, killSound[GetRandomInt(0, numSets - 1)][j]);
			strcopy(comboSound[numSets][j], PLATFORM_MAX_PATH, comboSound[GetRandomInt(0, numSets - 1)][j]);
		}
		strcopy(grenadeSound[numSets], PLATFORM_MAX_PATH, grenadeSound[GetRandomInt(0, numSets - 1)]);
		strcopy(selfkillSound[numSets], PLATFORM_MAX_PATH, selfkillSound[GetRandomInt(0, numSets - 1)]);
		strcopy(roundplaySound[numSets], PLATFORM_MAX_PATH, roundplaySound[GetRandomInt(0, numSets - 1)]);
		strcopy(knifeSound[numSets], PLATFORM_MAX_PATH, knifeSound[GetRandomInt(0, numSets - 1)]);
		strcopy(firstbloodSound[numSets], PLATFORM_MAX_PATH, firstbloodSound[GetRandomInt(0, numSets - 1)]);
		strcopy(teamkillSound[numSets], PLATFORM_MAX_PATH, teamkillSound[GetRandomInt(0, numSets - 1)]);
		strcopy(joinSound[numSets], PLATFORM_MAX_PATH, joinSound[GetRandomInt(0, numSets - 1)]);
		
		for (int j = 0; j < MAX_NUM_KILLS; j++) {
			headshotConfig[numSets][j] = headshotConfig[GetRandomInt(0, numSets - 1)][j];
			killConfig[numSets][j] = killConfig[GetRandomInt(0, numSets - 1)][j];
			comboConfig[numSets][j] = comboConfig[GetRandomInt(0, numSets - 1)][j];
		}
		grenadeConfig[numSets] = grenadeConfig[GetRandomInt(0, numSets - 1)];
		selfkillConfig[numSets] = selfkillConfig[GetRandomInt(0, numSets - 1)];
		roundplayConfig[numSets] = roundplayConfig[GetRandomInt(0, numSets - 1)];
		knifeConfig[numSets] = knifeConfig[GetRandomInt(0, numSets - 1)];
		firstbloodConfig[numSets] = firstbloodConfig[GetRandomInt(0, numSets - 1)];
		teamkillConfig[numSets] = teamkillConfig[GetRandomInt(0, numSets - 1)];
		joinConfig[numSets] = joinConfig[GetRandomInt(0, numSets - 1)];
	}
}
...
public EventRoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    // We call this for all games now to ensure global variables reset
    NewRoundInitialization();
    ...
    if (g_hMPGameMode != INVALID_HANDLE)
    {
        ...
        if (!StrEqual(g_sGameMode, "versus", false))
            g_bCoopMode = true;
    }
}
...
public NewRoundInitialization()
{
    ...
    for(new i = 1; i <= MaxClients; i++) 
    {
        ...
        lastKillTime[i] = -1.0;
        g_LastHitGroup[i] = 0; // Reset hitgroup tracking for the new round
    }
}
...
public OnClientPostAdminCheck(client)
{
    ...
    g_LastHitGroup[client] = 0 // Reset hitgroup tracking on join
    
    if(!IsFakeClient(client))
    {
        // Hook the client to track hitgroups (required for HL2DM headshots)
        SDKHook(client, SDKHook_TraceAttack, OnTraceAttack);

        soundPreference[client] = GetConVarInt(FindConVar("sm_quakesoundsv3_sound")) -1
        if (soundPreference[client] == -2) soundPreference[client] = numSets > 0 ? numSets : -1;
        textPreference[client] = GetConVarInt(FindConVar("sm_quakesoundsv3_text"))
    ...
    else
    {
        if(attackerClient == victimClient || attackerClient == 0)
        {
            ...
            for(new i = 1; i <= GetMaxHumanPlayers(); i++)
            {
                if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] >= 0)
                {
                    if (!g_bCoopMode || (g_bCoopMode && GetClientTeam(victimClient) == 2 ))
                    {
                        ...
                        if(textPreference[i])
                        {
                            // HL2 translation logic
                            new String:phrase[64];
                            if (strcmp(setsName[soundPreference[i]], "hl2") == 0) strcopy(phrase, sizeof(phrase), "hl2_selfkill");
                            else strcopy(phrase, sizeof(phrase), "selfkill");

                            if(selfkillConfig[soundPreference[i]] & 8)
                            {
                                PrintCenterText(i,"%t",phrase,victimName)
                            }
        ...
        else
        {
            ...
            {
                headshot = (g_LastHitGroup[victimClient] == 1);
            }
            ...
            for(new i = 1; i <= GetMaxHumanPlayers(); i++)
            {
                if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] >= 0)
                {
                    new bool:isHL2 = (strcmp(setsName[soundPreference[i]], "hl2") == 0);
                    ...
                    else if(knife && knifeConfig[soundPreference[i]] > 0)
                    {
                        ...
                        if(textPreference[i])
                        {
                            // HL2 translation logic
                            new String:phrase[64];
                            if (isHL2) strcopy(phrase, sizeof(phrase), "hl2_knife");
                            else strcopy(phrase, sizeof(phrase), "knife");

                            if(knifeConfig[soundPreference[i]] & 8)
                            {
                                PrintCenterText(i,"%t",phrase,attackerName,victimName)
                            }
                    ...
                    else if(grenade && grenadeConfig[soundPreference[i]] > 0)
                    {
                        ...
                        if(textPreference[i])
                        {
                            // HL2 translation logic
                            new String:phrase[64];
                            if (isHL2) strcopy(phrase, sizeof(phrase), "hl2_grenade");
                            else strcopy(phrase, sizeof(phrase), "grenade");
                    ...
                    else
                    {
                        ...
                        if(textPreference[i] && consecutiveKills[attackerClient] < MAX_NUM_KILLS)
                        {
                            // HL2 translation logic
                            new String:phrase[64];
                            if (isHL2) Format(phrase, sizeof(phrase), "hl2_killsound %i", consecutiveKills[attackerClient]);
                            else Format(phrase, sizeof(phrase), "killsound %i", consecutiveKills[attackerClient]);

                            if(killConfig[soundPreference[i]][consecutiveKills[attackerClient]] & 8)
                            {
                                PrintCenterText(i,"%t",phrase,attackerName)
                            }
...
// Test menu to play selected sound to self
public Action:CMD_ShowQuakePlayMenu(client,args)
{
	new String:buffer[100]
	new Handle:menu = CreateMenu(MenuHandlerQuakePlay)
...
public MenuHandlerQuakePlay(Handle:menu,MenuAction:action,param1,param2)
{
	if(action == MenuAction_Select)	
	{
		CreateMenuSoundList(param1,param2);
	} 
...
void CreateMenuSoundList(int client, int numSet, int iFirstIndex = 0)
{
	new String:buffer[100]
	new Handle:menu = CreateMenu(MenuHandlerQuakePlaySound)
	for (int j = 0; j < MAX_NUM_KILLS; j++) {
		if(strlen(headshotSound[numSet][j]) > 0) {
			Format(buffer,50,"headshot[%i]",j);
			AddMenuItem(menu, headshotSound[numSet][j],buffer);
		}
	}
	for (int j = 0; j < MAX_NUM_KILLS; j++) {
		if(strlen(killSound[numSet][j]) > 0) {
			Format(buffer,50,"kill[%i]",j);
			AddMenuItem(menu, killSound[numSet][j],buffer);
		}
	}
	for (int j = 0; j < MAX_NUM_KILLS; j++) {
		if(strlen(comboSound[numSet][j]) > 0) {
			Format(buffer,50,"combo[%i]",j);
			AddMenuItem(menu, comboSound[numSet][j],buffer);
		}
	}	
	AddMenuItem(menu, grenadeSound[numSet], "grenade");
	AddMenuItem(menu, selfkillSound[numSet], "selfkill");
	AddMenuItem(menu, roundplaySound[numSet], "roundplay");
	AddMenuItem(menu, knifeSound[numSet], "knife");
	AddMenuItem(menu, firstbloodSound[numSet], "firstblood");
	AddMenuItem(menu, teamkillSound[numSet], "teamkill");
	AddMenuItem(menu, joinSound[numSet], "join");
	
	SetMenuExitBackButton(menu, true);
	DisplayMenuAtItem(menu,client,iFirstIndex,MENU_TIME_FOREVER);
	g_iSelectedSet[client] = numSet;
}

public MenuHandlerQuakePlaySound(Handle:menu,MenuAction:action,param1,param2)
{
	if(action == MenuAction_Select)
	{
		char sPath[PLATFORM_MAX_PATH];
		GetMenuItem(menu, param2, sPath, sizeof(sPath));
		PrintToChat(param1, "Playing sound: %s", sPath);
		EmitSoundCustom(param1,sPath,_,_,_,_,GetConVarFloat(FindConVar("sm_quakesoundsv3_volume")));
		CreateMenuSoundList(param1, g_iSelectedSet[param1],GetMenuSelectionPosition());
	} 
	else if(action == MenuAction_End)
	{
		CloseHandle(menu)
		...
		if (param2 == MenuCancel_ExitBack)
			CMD_ShowQuakePlayMenu(param1, 0);
}
...
public ShowQuakeMenu(client)
{
	...
	{
		Format(buffer,100,"%T (%T)","no quake sounds",client, "enabled", client)
	}
	else
	{
	...
	for(new set = 0; set < numSets; set++) 
	{
		...
		{
			Format(buffer,50,"%T (%T)",setsName[set],client, "enabled", client)
		}
		else
		{
			Format(buffer,50,"%T",setsName[set],client)
	...
	if(soundPreference[client] == numSets) {
		Format(buffer,100,"%T (%T)","sound mixed",client, "enabled", client)
	}
	else {
		Format(buffer,100,"%T","sound mixed",client)
	}
	AddMenuItem(menu,"sound mixed",buffer)
...
// This function resets the player's hitgroup data when they respawn
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
    new client = GetClientOfUserId(GetEventInt(event, "userid"));
    if (client > 0 && client <= MaxClients)
    {
        g_LastHitGroup[client] = 0; // Clear hit data when the player respawns
    }
}

// This function tracks which part of the body was hit during an attack
